...
```
   Description<br>-----------<br>Gets a script to create the full database schema for the Citrix Host Service and copies it to a file called "C:\HostSchema.sql"<br>This script can be used to create the service schema in a database with name "MySiteDB", which must already exist, and must not already contain a Host service schema.
### EXAMPLE 2
```
C:\PS>Get-HypDBSchema -DatabaseName MySiteDB -ScriptType Login > C:\HostLogins.sql
```
   Description<br>-----------<br>Gets a script to create the appropriate database server logon for the Host service. This can be used when configuring a mirror server for use.
